...
# this dockerfile builds a test environment for PyCondor
...
RUN cd /tmp \
 ...
 && conda install -c conda-forge python=${PYTHON_VERSION} python-graphviz codecov \
 && conda update -y --all \
 && conda clean -y -all

# install pycondor dependencies early for caching
COPY requirements.txt /home/${SUBMIT_USER}/requirements.txt
COPY requirements_dev.txt /home/${SUBMIT_USER}/requirements_dev.txt
RUN pip install --no-cache -r /home/${SUBMIT_USER}/requirements.txt \
 && pip install --no-cache -r /home/${SUBMIT_USER}/requirements_dev.txt \
...
# copy HTCondor configs into place
COPY docker/condor_config.local /etc/condor/condor_config.local

# copy pycondor package into container and install it
# this is the only part that can't be cached against editing the package
COPY --chown=mapper:mapper . /home/${SUBMIT_USER}/pycondor
RUN chmod +x /home/${SUBMIT_USER}/pycondor/docker/entrypoint.sh
